 ...
 *  Use JavaCL_PathGenerator
 *  
 * @author yininggao
 *
 */
public class JavaCL_SimulationManager {
	
	private double tolerance=0.1;
	private JavaCL_PathGenerator generator;
	 ...
	 * @param option
	 */
	public JavaCL_SimulationManager (Option option) {
		generator = new JavaCL_PathGenerator (option);		// Create a new JavaCL_PathGenerator	
		stats = new StatsCollector();                       // Create a new StatsCollector
		r = option.getInterestRate();
		period = option.getPeriod();
		strickPrice = option.getStrickPrice(); 
	}
	...
	public double getPrice(){
		while(!isFinished()){
			// launch two batch 
			double[] price = generator.getLastDayPrice();
			for (double p :price) {
				stats.add(Math.max(p-strickPrice,0));
			}
		}
